...
$files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($sources['vendor_dir']), RecursiveIteratorIterator::SELF_FIRST);
foreach ($files as $pathname => $file) {
    if (is_dir($pathname))
        continue;
    $modx->log(modX::LOG_LEVEL_INFO, 'Adding composer dependencies file ' . $pathname);
    $dir = str_replace($root . "core", "", dirname($pathname) . '/');
    $vehicle->resolve('file', array(
        'source' => $pathname,
        'target' => "return MODX_CORE_PATH . '{$dir}';",
